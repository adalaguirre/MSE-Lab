...
// Declaring the External symbol declarations
...
﻿  SystemInit();
    ...
    // Initialize pointers to ythe soucer and detination of the .data section
    uint32_t *p_src_mem = (uint32_t *)&_etext;
    uint32_t *p_dest_mem = (uint32_t *)&_sdata;

    // Transferring the .data section
    for (uint32_t i = 0; i < data_mem_size; i++){
        *p_dest_mem++ = *p_src_mem++; // The ++ means that it's increasing the pointer address so it points to the next element
    }
    ...
    // Call the applicaciton's main fucntion
    main();
}